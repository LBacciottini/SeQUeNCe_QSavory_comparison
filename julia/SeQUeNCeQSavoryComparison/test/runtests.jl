...
const DEFAULT_CONFIG = joinpath(REPO_ROOT, "shared", "configs", "default.toml")
const WERNER_CASES = joinpath(REPO_ROOT, "shared", "testdata", "werner_protocol_cases.toml")
...
function _werner_cases()
    ...
    for raw_line in readlines(WERNER_CASES)
        line = strip(raw_line)
